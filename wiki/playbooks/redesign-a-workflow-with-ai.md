...
4. 重新設計流程，而不是把舊 SOP 原樣搬進 AI
5. 用小規模試點驗證是否真的節省時間、改善質量或提升可用性
6. 若場景受限或高風險，補上 rule engine、告警與人工邊界，不把所有判斷責任壓在模型上
7. 在探索型任務中，保留多樣候選與非最優方案，避免流程過早鎖死在單一路徑
8. 若流程的痛點來自一線執行不穩、偷懶或尋租，不要只優化前台介面，應把提醒、驗證、派單與異常升級一起重畫
