...
16. 若流程發生在機場、工廠、倉庫、醫院等物理現場，不要只畫軟體 SOP；先盤點場地能否改造、哪些動線是為人設計、哪些任務可拆給機器人、人工如何接管，以及故障、維護和事故責任如何回到流程裡。
17. 若 agent 可以把既有 App 當成「慢 API」來操作，不要只在有無官方 API 之間二選一；把瀏覽器操作、正式 API、人工確認、權限隔離、審計紀錄與失敗回滾一起設計成可驗證流程。
18. 若你想把 AI workflow 做成可收費的一人公司產品，順序應是先用手工方式驗證付費需求，再自動化已被證明重複且穩定的環節；不要先把完整系統做完才找市場。
...
- 手工驗證不是所有場景都適用；涉及高風險資料、合規或人身安全時，早期試錯也要先設邊界。
