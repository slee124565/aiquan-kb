...
- 評估 AI 基礎設施與模型生態時，不能只看晶片性能或單次跑分；還要同步看軟體棧、公開 benchmark、供應鏈依賴、資料中心電力/冷卻/施工瓶頸、客戶綁定與政策副作用。
- 評估 agent 工具時，不能只看功能清單或 demo 酷不酷；要測它在未知格式、缺工具、封閉介面與上下文缺口前，是否能找替代通道、保留可審核線索，並在需要時向人求證。
