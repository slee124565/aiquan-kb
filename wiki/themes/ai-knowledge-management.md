# AI Knowledge Management

## Summary

這個主題聚焦在 AI 時代如何把知識從「收藏」推進到「可被 agent 與人共同使用的外部化工作層」。

## Why It Matters

在 `AI学习圈 / 快刀廣播站` 這個內容域裡，知識管理不是邊角話題，而是連接工具、工作流、學習與長期複利的核心主題。

## Current Framing

- 好的知識庫不是資料倉庫，而是可被檢索、引用、回寫與持續更新的工作結構
- 降低幻覺的關鍵，往往不是更強模型，而是更可信的來源層
- AI 時代的知識管理，重心從手工整理轉向來源選擇、問題設計與回寫機制
- 來源選擇本身需要判準：值得長期保存的材料應能看見作者的一手觀察、思考證明、動機邊界與現實影響，而不是只因為它被 AI 整理得完整
- 當材料量超過單次上下文容量時，知識庫需要中間索引層；AI 生成的 entity / concept / relationship 圖譜可以幫人與 agent 先定位該讀哪些來源
- 個人與團隊知識庫的差異，常不在工具，而在維護責任與使用情境
- 當知識庫進入醫療、法律、財務等高風險生活決策時，最小骨架必須包含來源、時間線、個人化上下文、可追問問題與人工核驗路徑；它的價值不是收藏資料，而是讓人更有能力參與專業決策

## Representative Broadcasts

- [416](../broadcasts/416.md)
- [350](../broadcasts/350.md)
- [351](../broadcasts/351.md)
- [393](../broadcasts/393.md)
- [396](../broadcasts/396.md)
- [398](../broadcasts/398.md)
- [422](../broadcasts/422.md)
- [450](../broadcasts/450.md)
- [786](../broadcasts/786.md)
- [812](../broadcasts/812.md)
- [445](../broadcasts/445.md)
- [473](../broadcasts/473.md)
- [496](../broadcasts/496.md)
- [524](../broadcasts/524.md)
- [553](../broadcasts/553.md)
- [574](../broadcasts/574.md)
- [598](../broadcasts/598.md)
- [642](../broadcasts/642.md)
- [657](../broadcasts/657.md)
- [682](../broadcasts/682.md)
- [701](../broadcasts/701.md)
- [716](../broadcasts/716.md)
- [834](../broadcasts/834.md)
- [special-20260429-ai-era-reading-standards](../broadcasts/special-20260429-ai-era-reading-standards.md)
- [special-20260429-ai-knowledge-graph](../broadcasts/special-20260429-ai-knowledge-graph.md)

## Related Playbooks

- [Build An AI-Assisted Knowledge Base](../playbooks/build-an-ai-assisted-knowledge-base.md)

## Related Series

- [教程上新](../series/tutorial-launches.md)

## Open Questions

- 知識庫中哪些內容應由人維護，哪些可交給 agent
- 個人知識管理與團隊知識管理，最小共同骨架是什麼
- 高風險個人資料庫是否需要獨立 playbook，避免把一般筆記方法直接套到醫療、法律或財務場景
- 什麼時候需要把一般索引升級為 entity / relationship 層，避免知識圖譜只變成漂亮但低用的視覺化

## Related Pages

- [AI Knowledge Management](../maps/ai-knowledge-management.md)
